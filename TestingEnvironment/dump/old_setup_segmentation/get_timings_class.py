...
class GetTimings(object):
    ...
    def __init__(self, video_dir, sub_clip_dir):
        ...
        self.initialise_extracting_data(self.fvs, self.id_extractor)


    def initialise_variables(video_dir, sub_clip_dir):
    ...
    def initialise_reading_frames(video_dir, id_extractor):
        # start grabbing frames
        self.fvs = FileVideoStream(self.video_dir, self.id_extractor).start()

    # initialise all the threads to extract the data
    def initialise_extracting_data(fvs, id_extractor):
        self.lock = Lock()
        for i in range(self.Nr_Threads):
            self.fvr = FileVideoRead(fvs, i, self, id_extractor).start()
    ...
    def _frame_number_to_time(self, dictionary):
        ...
        for topic in dictionary:
            time = math.floor(self.convert_frame_num_to_time( frame_number=dictionary[topic], frame_rate=self.frame_rate))
            times_array.append(time)
        # check if the last fraame is in times array, if not then add it
        last_frame_time = math.floor(self.convert_frame_num_to_time( frame_number=self.fvs.frame_number, frame_rate=self.frame_rate))
        print('dictionary = {}\ntimes_array = {}\nlast frame number = {} last_frame_time={}'.format(dictionary,times_array,self.fvs.frame_number,last_frame_time))
    ...
    def stop(self):
        ...
        print('self.time_finish-self.time_start={}'.format(self.time_finish-self.time_start))
        ...
        print (times_array)
        print ('$' * 60)
        course_name = self.get_course_name(self.dictionary_frame_text)
