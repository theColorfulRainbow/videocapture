...
class Segmentor(object):
    ...
    def initialise_extracting_data(fvrf, id_extractor):
        self.lock = Lock()
        # save the fvr into a list
        for i in range(self.Nr_Threads):
            self.fvrd.append(FileVideoReadData(fvrf, i, self, id_extractor))
    ...
    def _frame_number_to_time(self, dictionary):
        ...
        last_frame_time = math.floor(self._convert_frame_num_to_time( frame_number=self.fvs.frame_number, frame_rate=self.frame_rate))
    ...
    def stop(self):
        ...
        ordered_dictionary = self.__orderDictionary(self.dictionary_frame_text)
        print ('%' * 60)
