...
from FileVideoStreamText_testing_QR import FileVideoRead
...
class GetTimings(object):
    ...
    # loop over frames from the video file stream
        ...
        while self.fvs.more():
            ...
            # ----------------------- TESTING QR  --------------------------------
            frame_txt = 'NO QR DETECTED'
            decodedFrame = pyzbar.decode(frame)
            for obj in decodedFrame:
                frame_txt = obj.data.decode('utf-8')
                print(frame_txt)  
            #---------------------------------------------------------------------
...
if __name__ == "__main__":
    cwd = os.getcwd()
    timings_class = GetTimings('test_QR_13s.mp4',cwd)
    timings_class.start() 